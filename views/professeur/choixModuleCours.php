...
if (isset($_SESSION['user_data'])) {
    if ($_SESSION['user_data']['role'] == 3 || $_SESSION['user_data']['role'] == 2  || $_SESSION['user_data']['role'] == 1) {
        $id = $_SESSION['user_id'];
        $qr = mysqli_query($conn,"SELECT a.*, m.name as mname, m.id as id
                                  FROM affectationmoduleprof a
                                  JOIN users u ON u.id = a.id_teacher
                                  JOIN module m ON m.id = a.id_module
                                  WHERE u.id = $id");
        $data = [];
            ...
            <div class="page-wrapper">
                <div class="content container-fluid">
                    <div class="page-header">
                        <div class="row">
                            <div class="col">
                                <h3 class="page-title">Sélectionner Module</h3>
                                <ul class="breadcrumb">
                            ...
                            <?php foreach ($data as $d) { ?>
                                <div class="col-12 col-md-6 col-lg-4 d-flex" style="margin-left:15px">
                                    <div class="card flex-fill bg-white">
                                        ...
                                        <div class="card-body">
                                            <p class="card-text"><?php echo ucfirst($d['mname']) ?></p>
                                            <a class="btn btn-primary" href="/ENSAHify/views/professeur/attachements.php?id_module=<?php echo $d['id'] ?>">Accéder</a>
                                        </div>
                                    </div>
                                </div>
                            <?php } ?>  
